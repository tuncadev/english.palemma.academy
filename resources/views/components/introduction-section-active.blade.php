...
<li class="rounded-lg rounded-tl-none rounded-bl-none border-r-2 border-y-2 border-gray-200 bg-sky-100">
    <div class="ms-6 py-3">
        ...
        <h3 class="font-medium leading-tight">Introduction</h3>
            <a class="button_steps_sections mt-2 {{ Route::currentRouteName() === 'course.coure_intro' ? 'bg-sky-200' : ' bg-white' }}" href="{{ route('course.introduction', ['course_id' => $course_id, 'section_id' => $current_id]) }}"> Course Intro</a>
            <a class="button_steps_sections mt-2 {{ Route::currentRouteName() === 'course.course_instructions' ? 'bg-sky-200' : ' bg-white' }}" href="{{ route('course.instructions', ['course_id' => $course_id, 'section_id' => $current_id]) }}">Instructions</a>
    </div>
</li>
