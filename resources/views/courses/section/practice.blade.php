        ...
        <div class=" sm:ml-64">
            <div class="mt-4 rounded-lg">
                <div class="p-2 flex flex-col items-center justify-center mb-4 gap-6 rounded bg-gray-50 dark:bg-gray-800">
                    ...
                    @endif
                    <div class="flex">
                        <button data-url="{{ route('course.show', ['course_id' => $course_id, 'section_id' => $section['id']]) }}?colorClass={{$colorClass}}"
                            onclick="redirectToSection(this)"
                            class="relative flex items-center gap-3 text-sm bg-gradient-to-b from-pink-500 to-orange-500 text-white py-5 px-4 font-semibold shadow-md rounded-lg">
                            <span class="text-lg capitalize md:pl-8"> {{$courseName}} - {{ $sectionName }}</span>
                        </button>
                    </div>
